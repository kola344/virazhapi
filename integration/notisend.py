...
class SMS:
	...
	async def doRequest (self, rqData, url):
		...
		async with aiohttp.ClientSession() as session:
			async with session.get(self.url+url, headers=self.headers, params=rqData, ssl=False) as response:
...
#									#
	...
	async def getBalance(self):		# узнаём баланс
		rqData = {}
		return await self.doRequest(rqData, 'message/balance')
			
	async def messagePrice(self, recipients, message):		# узнаём цену сообщений
		rqData = {
			"recipients": recipients,
			"message" 	: message,
			"sender" 	: self.sender
		} 
		return await self.doRequest(rqData, 'message/price')
	
	async def info(self, phones ):	# узнаём информацию о номерах
		rqData = {"phones" : phones} 
		return await self.doRequest(rqData, 'message/info')
	
	async def statusSMS(self, messages_id):	# узнаём статус SMS
		rqData = {"messages_id" : messages_id} 
		return await self.doRequest(rqData, 'message/status')
		
#									#
#			РАССЫЛКИ				#
#									#
	async def create(self, include, message, exclude=0, sender='', run_at='', slowtime='', slowsize='', name='', test=0):	# Создание смс рассылки
		rqData = {
			"include"	: include,
			"message"	: message
		}
		...
		return await self.doRequest(rqData, 'sending/create')

	async def groups(self, type):	# Запрос групп
		rqData = {"type" : type} 
		return await self.doRequest(rqData, 'sending/groups')

	async def status(self, id):	# Запрос статуса рассылки
		rqData = {"id" : id} 
		return await self.doRequest(rqData, 'sending/status')
