...
class orders:
    ...
    async def add_order(self, data, delivery_at, comment, order_by, address, date, price, payment):
        cursor = await self.db.execute('INSERT INTO orders (data, delivery_at, comment, order_by, address, status, closed, date, price, payment) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (str(data), delivery_at, comment, order_by, address, "🕙 В обработке", 0, date, price, payment))
        await self.db.commit()
    ...
    async def get_orders_history(self, key):
        ...
        for order in fetched:
            orders.append({"order_id": order[0], "data": eval(order[1]), "text": order[2],
                           ...
                           "address": order[6], "status": order[7], "date": order[10], "price": order[11]})
        print(orders)
        return orders
